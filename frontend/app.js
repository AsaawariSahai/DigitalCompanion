...
async function callClaude(msg,mode){
  S.pet.beh.talkCount++;
  if(mode==='study')S.pet.beh.studyCount=(S.pet.beh.studyCount||0)+1;
  if(mode==='emotional')S.pet.beh.eCount=(S.pet.beh.eCount||0)+1;
  addBond(2);
  checkEvo();

  const msgs=[...S.pet.mem.convos.slice(-16),{role:'user',content:msg}];

  try{
    const res=await fetch("http://localhost:3000/api/chat",{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({messages:msgs,system:buildPrompt(mode)})
    });

    const d=await res.json();
    const reply=d.reply || d.content?.[0]?.text || '...';

    S.pet.mem.convos.push({role:'user',content:msg},{role:'assistant',content:reply});
    if(S.pet.mem.convos.length>40)S.pet.mem.convos=S.pet.mem.convos.slice(-40);
    ...
    return '*static hiss* ...connection wavered...';
  }
}
